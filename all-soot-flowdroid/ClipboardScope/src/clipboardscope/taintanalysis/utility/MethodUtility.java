...
public class MethodUtility {
	...
	public static ThisRef getThisRef(SootMethod smthd) {
		for (ValueBox box : smthd.retrieveActiveBody().getUseAndDefBoxes()) {
			if (box.getValue() instanceof ThisRef) {
					return (ThisRef) box.getValue();
			}
		}
	...
	public static String getMthdTemplt(SootMethod smthd) {
		return smthd.getReturnType().toString() + smthd.getName().toString() + smthd.getParameterTypes().toString();
	}
	
	public static boolean isVariable(Value r) {
		return r instanceof FieldRef && ((FieldRef) r).getField().getDeclaringClass().isApplicationClass();
	}
}
